	...
	const statCards = $derived([
		{
			label: 'Makers',
			value: data?.maker ?? 0,
			icon: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg>`,
			color: 'indigo',
			href: '/maker'
		},
		{
		...
		},
		{
			label: 'Plugins',
			value: data?.plugin ?? 0,
			icon: `<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>`,
			color: 'rose',
			href: '/plugin'
		}
	]);
...
<div class="page">
	<header class="page-header">
		...
		<div class="status-dot" class:active={!loading && !error} title={loading ? 'Updating…' : error ? 'Connection error' : 'Live'}></div>
	</header>

	<!-- Stat cards -->
	<div class="stat-grid">
		{#each statCards as card}
			<a href={card.href} class="stat-card stat-{card.color}">
			...
			{#if data && data.eps > 0}
				<div class="progress-wrap">
					<div class="progress-bar">
						<div
							class="progress-fill"
							style="width: {Math.min(100, Math.round((data.actualEps / data.eps) * 100))}%"
						></div>
					</div>
					<span class="progress-pct">{Math.min(100, Math.round((data.actualEps / data.eps) * 100))}%</span>
				</div>
			{/if}
		</div>
